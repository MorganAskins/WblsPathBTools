...
void prunefile(string iname, string oname, double wallradius, double wallheight);
...
int main(int argc, char** argv)
{
  ...
  double z = argc == 5 ? stod(argv[4]) : radius;
  prunefile(argv[1], argv[2], radius, z);
  return 0;
}
...
void prunefile(string iname, string oname, double wallradius, double wallheight)
{
  ...
  for( int i=0; i < entries; i++ )
  {
    ...
    for(int evc=0; evc<ds->GetEVCount(); evc++)
    {
      ...
      double dw = dwall(pos, wallradius, wallheight);
